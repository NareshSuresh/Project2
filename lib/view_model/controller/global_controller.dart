...
class GlobalController extends GetxController {
  ...
  final RxString city="Mysuru".obs;
  ...
  Future<void> fetchWeatherByCity(String cityName) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Update the observable fields
        _weatherData.value = WeatherModel.fromJson(data);
        _weatherMain.value = data['weather'][0]['main'] ?? 'Unknown';
        latitude.value = data['coord']['lat'] ?? 0.0;
        longitude.value = data['coord']['lon'] ?? 0.0;
        city.value=cityName;
        print('Weather updated successfully for city: $cityName');
      } else {
        print(response.body);
